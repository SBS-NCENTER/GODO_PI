...
**Grid overlay** (`drawGrid` in `lib/overlayDraw.ts`): world-frame anchored. Zoom-adaptive interval schedule (`GRID_INTERVAL_SCHEDULE`): zoom < 0.3 px/m → 5 m, ≤ 1 → 1 m, ≤ 3 → 0.5 m, otherwise → 0.1 m. Lines per axis cap `GRID_MAX_LINES_PER_AXIS = 200` (extreme-zoom tab freeze 방지).

**Origin/Axis overlay** (`drawOriginAxis` in `lib/overlayDraw.ts`): YAML origin을 dot으로, +x를 빨강 (`AXIS_X_COLOR = '#dc2626'`), +y를 초록 (`AXIS_Y_COLOR = '#16a34a'`)으로 ROS REP-103 convention으로 그림. `yamlOriginYawDeg`로 회전 — Apply 후 운영자가 시각적으로 회전 적용 여부 확인. 호출 site는 `<PoseCanvas>`의 `drawPoseLayer` 내부 + `<MapEdit>`의 MapUnderlay `ondraw` hook. 핀 `tests/unit/overlayDraw.test.ts`.
...
- Standalone `<OriginAxisOverlay>` / `<GridOverlay>` 컴포넌트는 issue#28.1 (2026-W19)에서 삭제됨; PoseCanvas의 `drawPoseLayer` + MapEdit의 MapUnderlay `ondraw` hook이 단일 sole owner.
