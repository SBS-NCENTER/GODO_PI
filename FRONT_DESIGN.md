...
#### I6. Pinch-zoom carve-out (issue#2.2, 2026-04-30 HIL)

Mouse-wheel zoom은 여전히 금지 (I4 그대로) — 트랙패드 핀치 제스처만 carve-out으로 허용. 브라우저는 핀치를 `wheel` 이벤트 + `e.ctrlKey === true` (synthetic, 사용자가 실제로 Ctrl 안 눌러도)로 매핑. `MapUnderlay.svelte` + `MapMaskCanvas.svelte`의 `onwheel` 핸들러 둘 다 `ctrlKey` 게이트로 일반 스크롤과 구별. 평범한 스크롤은 그대로 페이지 스크롤. 핀치 감도는 fractional — 한 wheel 이벤트당 `factor = MAP_ZOOM_STEP ^ (-deltaY / MAP_PINCH_DELTA_PX_PER_STEP)` (100 px = 1 step). 한 제스처 (~20 events × 10 px)가 ~1.5× zoom으로 자연스럽게 끝나도록 튜닝됨 (HIL: "줌인/줌아웃이 너무 빠르게 되서 원하는 확대 비율에 정착하기 어려워" → 100 px/step 잠금).

#### I7. Map page common header (issue#2.4, 2026-04-30 HIL)

`/map` 페이지의 sub-tab 독립 헤더. `<TrackerControls/>` (Mode chip + Calibrate / Live / Backup buttons) + `<LastPoseCard/>` (x/y/yaw/σ_xy/converged readout) — sub-tab 행 위에 single-mount, Overview/Edit 어느 sub-tab이든 같이 보임. Operator phrasing: "TrackerControls + LastPoseCard도 sub-tab 상관없이 항상 보이게". `<ScanToggle/>` — sub-tab 행의 우측 끝 single-mount, sub-tab 무관하게 동일 위치 (operator-locked: "같은 기능을 하는 동일 모듈이니"). Overview body는 `<PoseCanvas/>` ABOVE `<MapListPanel/>` — operator HIL: "map 미리보기 블럭이 위로, 맵 목록 블럭이 아래로". 기존 `Dashboard.svelte`의 inline copy는 그대로 남아있음 (incremental rollout — HIL 검증 후 별도 follow-up에서 swap 가능).
