...
describe('projectScanToWorld — Mode-A TB2 polar→Cartesian cases', () => {
  ...
  it('case 3: yaw=0° + beam at 90° + r=1 → (pose_x, pose_y + 1)', () => {
    // Same expected output as case 1, but the operands are swapped
    // (yaw=0, beam=90 vs. yaw=90, beam=0). Catches yaw-vs-beam-angle
    // confusion bugs.
    const scan = makeScan({
      pose_x_m: 5,
      pose_y_m: 7,
...
describe('projectScanToWorld — gating', () => {
  ...
  it('returns empty when pose_valid=0 (Mode-A M3 — non-converged anchor)', () => {
    const scan = makeScan({ pose_valid: 0 });
    expect(projectScanToWorld(scan)).toEqual([]);
  });
...
describe('projectScanToWorld — Mode-A TB1 positional integrity', () => {
  ...
  // index i is exactly (r × cos(a), r × sin(a)) with r and a from the
  // formula. We verify three sample indices to pin the contract.
