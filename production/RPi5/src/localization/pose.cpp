...
double circular_mean_yaw_deg(const Particle* particles,
    ...
    double mean_deg = mean_rad * godo::constants::KRAD_TO_DEG;
...
double circular_std_yaw_deg(const Particle* particles,
    ...
    if (R >= 1.0) return 0.0;
    if (R <= 0.0) return std::sqrt(-2.0 * std::log(1e-300)) * godo::constants::KRAD_TO_DEG;
    const double std_rad = std::sqrt(-2.0 * std::log(R));
    return std_rad * godo::constants::KRAD_TO_DEG;
}
