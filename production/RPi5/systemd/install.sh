...
echo "[5/12] Installing udev rule for /dev/rplidar symlink (issue#10 + issue#10.1)"
...
# ttyUSB enumeration order. issue#10.1 — operator-tunable serial via
# `serial.lidar_udev_serial` in tracker.toml; install.sh sed-renders
# `99-rplidar.rules.template` into the final rule file. Re-run the
# script after a serial change. Production tracker.toml's
# `serial.lidar_port` should read `/dev/rplidar` (schema default);
# dev hosts without the studio's specific cp210x serial must
# override `[serial] lidar_udev_serial = "<dev-host-serial>"` AND/OR
# `[serial] lidar_port = "/dev/ttyUSB0"` in their tracker.toml.

LIDAR_UDEV_SERIAL_DEFAULT="2eca2bbb4d6eef1182aae9c2c169b110"
LIDAR_UDEV_SERIAL=""
if [[ -e "$TRACKER_TOML" ]]; then
    # tomllib (Python 3.11+) preserves whitespace-resilient parsing;
    # falls back to default on missing key / parse error. Mirrors the
    # justification at the existing tomllib block above (mapping ladder).
    LIDAR_UDEV_SERIAL=$(python3 - "$TRACKER_TOML" 2>/dev/null <<'PYEOF'
import sys
import tomllib
try:
    ...
    sys.exit(0)
v = doc.get("serial", {}).get("lidar_udev_serial")
if isinstance(v, str) and v:
    print(v)
PYEOF
)
fi
LIDAR_UDEV_SERIAL="${LIDAR_UDEV_SERIAL:-$LIDAR_UDEV_SERIAL_DEFAULT}"

# Strict format check — cp210x USB serial is exactly 32 lowercase hex.
# A wrong-format value produces a udev rule that will NEVER match.
if ! [[ "$LIDAR_UDEV_SERIAL" =~ ^[0-9a-f]{32}$ ]]; then
    echo "ERROR: install.sh: serial.lidar_udev_serial format invalid" >&2
    echo "       Got: '$LIDAR_UDEV_SERIAL'" >&2
    echo "       Expected: exactly 32 lowercase hex characters (cp210x factory serial)" >&2
    echo "" >&2
    echo "Manual action:" >&2
    echo "  1. Find the LiDAR's serial — System tab 도움말 card explains how:" >&2
    echo "     udevadm info -a -n /dev/ttyUSB0 | grep -m1 'ATTRS{serial}'" >&2
    echo "  2. Edit the value in the SPA Config tab → serial.lidar_udev_serial" >&2
    echo "     (or directly in $TRACKER_TOML if SSH is the only path)" >&2
    echo "  3. Re-run sudo bash production/RPi5/systemd/install.sh" >&2
    exit 1
fi

UDEV_TEMPLATE="$SCRIPT_DIR/99-rplidar.rules.template"
# Extend the existing trap (which already cleans MAPPING_UNIT_TMP) with
# the new udev temp file. Set the merged trap BEFORE mktemp so a SIGINT
# between the two lines does not leak the new temp. Use ${VAR:-} guard
# because UDEV_RENDERED is empty until mktemp runs.
trap 'rm -f "$MAPPING_UNIT_TMP" "${UDEV_RENDERED:-}"' EXIT
UDEV_RENDERED=$(mktemp /tmp/99-rplidar.rules.XXXXXX)

# Prepend a provenance comment so SSH inspection of the rendered file
# shows it is regenerated, not hand-edited. The template's own header
# already documents the source-tree side.
UDEV_RENDERED_TS=$(date '+%Y-%m-%d %H:%M:%S %Z')
{
    echo "# Generated by install.sh from 99-rplidar.rules.template at ${UDEV_RENDERED_TS}"
    sed -e "s/__LIDAR_SERIAL__/${LIDAR_UDEV_SERIAL}/g" "$UDEV_TEMPLATE"
} > "$UDEV_RENDERED"
install -m 0644 -o root -g root "$UDEV_RENDERED" /etc/udev/rules.d/99-rplidar.rules
...
echo "  → /etc/udev/rules.d/99-rplidar.rules generated with serial=${LIDAR_UDEV_SERIAL}"
echo "  → rules reloaded + tty add events triggered."
