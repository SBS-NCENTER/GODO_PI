...
TEST_CASE("HotConfig layout is exactly 40 bytes (Mode-A M1)") {
    static_assert(sizeof(HotConfig) == 40,
                  "HotConfig size pin moved without test update");
    CHECK(sizeof(HotConfig) == 40);
}

TEST_CASE("snapshot_hot copies the three hot fields from Config") {
    Config c = Config::make_default();
    c.deadband_mm           = 12.5;
    c.deadband_deg          = 0.25;
    c.amcl_yaw_tripwire_deg = 7.5;
    const auto h = snapshot_hot(c);
    CHECK(h.deadband_mm           == 12.5);
    CHECK(h.deadband_deg          == 0.25);
    CHECK(h.amcl_yaw_tripwire_deg == 7.5);
    CHECK(h.valid                 == 1);
...
TEST_CASE("snapshot_hot is deterministic — two calls yield identical layout") {
    ...
    CHECK(a.amcl_yaw_tripwire_deg == b.amcl_yaw_tripwire_deg);
